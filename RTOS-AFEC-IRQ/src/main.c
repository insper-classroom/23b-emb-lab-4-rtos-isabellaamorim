...
#define TASK_ADC_STACK_PRIORITY (tskIDLE_PRIORITY+1)
...
static void task_proc(void *pvParameters) {
	...
	while (1) {
		if (xQueueReceive(xQueueADC, &(adc), 1000)) {
			...
			xQueueSend(xQueueSendData, &media, &xHigherPriorityTaskWoken);
		}
	}
}
...
static void task_adc(void *pvParameters) {
	...
	while (1) {
		if (xQueueReceive(xQueueSendData, &media, 1000)) {
			printf("ADC MEDIA: %d \n", media); // Removed the ampersand before media.
		} else {
...
int main(void) {
	...
	xQueueSendData = xQueueCreate(100, sizeof(adcData));
	if (xQueueSendData == NULL)
	printf("falha em criar a queue xQueueSendData \n");
